...
export function SidePanel() {
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={open => { if (!open) dispatch({ type: 'SET_SELECTED', id: null }); }}>
      <Dialog.Portal container={typeof document !== 'undefined' ? (document.getElementById('main-area') ?? undefined) : undefined}>
        <Dialog.Overlay className="absolute inset-0 bg-black/40 z-40" />
        <Dialog.Content className="absolute right-0 top-0 h-full w-[640px] bg-[var(--panel)] border-l border-[var(--line)] z-50 overflow-hidden focus:outline-none">
          {state.selectedId && (
            <SubprocessPanel
